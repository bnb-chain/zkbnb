...
for val in $rpc; do
    ...
    echo "Docker Build & Push [${val}]: "
    declare -l lower="${val}"
    docker build -t --no-cache us-central1-docker.pkg.dev/zecrey-330903/zecrey-webhook/${lower}:$1 -f service/rpc/${val}/Dockerfile .
    docker push us-central1-docker.pkg.dev/zecrey-330903/zecrey-webhook/${lower}:$1
    docker image prune --filter label=stage=gobuilder --force
...
for val in $cronjob; do
    ...
    docker build -t --no-cache us-central1-docker.pkg.dev/zecrey-330903/zecrey-webhook/${lower}:$1 -f service/cronjob/${val}/Dockerfile .
