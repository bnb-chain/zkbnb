...
For convenience of user, ZKBAS implemented new version supporting metamask usage by taking advantages of
EIP-712 standard signature which is able to sign any structured message and supported well by the metamask apps.

Any user can be accessible to ZkBAS only if the user gets metamask apps, and any user can easily manage their L2 assets(NFT, Tokens, etc.)
by importing the ecdsa(secp256k1) private key to metamask.

![Metamask](./assets/metamask-usage.png)
...
EIP-712 is a standard for hashing and signing of typed structured data as opposed to just bytestrings. It includes a
...
EIP-712 standard is implemented in metamask as below

![Metamask](./assets/eip-brief-view.png)

And here is the encode and sign flow from bottom to top.
...
7. sign the hash(bytesConcat) using the privateKey, and generate final signature.

### Elliptic Curve 

For an adaptation of metamask support, ZKBAS switch ecc signature algorithm's elliptic curve from original eddsa to ecdsa, precisely secp256k1 which 
is used across EVM-based blockchain. Moreover in order to support ecdsa change ZKBAS switch the zksnarks proving system from groth16 to plonk, while the contract verifier switch so from groth16 verifier to plonk verifier.

### Hash Function

For an adaptation of metamask support, ZKBAS switch hash algorithm from originally MIMC to Keccak256(SHA-3).
...
### Encodes of Transaction

For an adaptation fo metamask support, ZKBAS switch encodes of transaction from originally concat all bytes to 
EIP-712 signed typed message encoding, as described at [Brief View of EIP-712 Implementation](#brief-view-of-eip-712-implementation). Basically the encode switched to a 
