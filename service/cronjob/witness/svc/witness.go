...
func (w *Witness) initState() {
	p, err := w.proofSenderModel.GetLatestConfirmedProof()
	if err != nil {
...
func (w *Witness) updateTimeoutUnprovedBlock() {
	latestConfirmedProof, err := w.proofSenderModel.GetLatestConfirmedProof()
	...
	_, err = w.proofSenderModel.GetProofByBlockNumber(nextBlockNumber)
	if err == nil {
		return
	}
