...
func (n *nft) GetNftListByAccountIndex(ctx context.Context, accountIndex, limit, offset int64) (nfts []*nftModel.L2Nft, err error) {
	...
	nftList := make([]*nftModel.L2Nft, 0)
	value, err := n.cache.GetWithSet(ctx, multcache.KeyGetAccountNftList+fmt.Sprintf("%d:%d:%d", accountIndex, limit, offset), &nftList, 1, f)
	if err != nil {
		return nil, err
	}
...
func (n *nft) GetAccountNftTotalCount(ctx context.Context, accountIndex int64) (int64, error) {
	...
	var count int64
	value, err := n.cache.GetWithSet(ctx, multcache.KeyGetAccountNftTotalCount+fmt.Sprintf("%d", accountIndex), &count, 5, f)
	if err != nil {
		return count, err
	}
