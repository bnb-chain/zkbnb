...
func TestGetAccountStatusByAccountNameLogic_GetAccountStatusByAccountName(t *testing.T) {
	...
	defer ctrl.Finish()
	mockAccount := account.NewMockAccountModel(ctrl)
	...
	// error case
	mockAccount.EXPECT().GetAccountByAccountName(gomock.Any(), gomock.Any()).Return(nil, zerror.New(-1, "error")).MaxTimes(1)
	req := &types.ReqGetAccountStatusByAccountName{AccountName: ""}
	_, err := l.GetAccountStatusByAccountName(req)
	assert.NotNil(t, err)

	// normal case
	mockAccount.EXPECT().GetAccountByAccountName(gomock.Any(), gomock.Any()).Return(&table.Account{}, nil).AnyTimes()
	req = &types.ReqGetAccountStatusByAccountName{AccountName: ""}
	_, err = l.GetAccountStatusByAccountName(req)
	assert.Nil(t, err)
}
