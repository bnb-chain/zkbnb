...
type (
	BlockModel interface {
		...
		CreateBlockAndUpdateMempoolAndUpdateAmount(block *Block, mempoolTxs []*mempool.MempoolTx, totalAmountMap map[int]map[int]int, txs []*tx.Tx) error
...
/*
	...
	Params: *Block
	Return: error
	Description: Update Block when committer completing packing new L2Block. And insert txs
*/
...
func (m *defaultBlockModel) CreateBlockAndUpdateMempoolAndUpdateAmount(block *Block, mempoolTxs []*mempool.MempoolTx, totalAmountMap map[int]map[int]int, txs []*tx.Tx) error {
	err := m.DB.Transaction(
		func(tx *gorm.DB) error { // transact
			// Create Block
			dbTx := tx.Table(m.table).Create(block)
			if dbTx.Error != nil {
				logx.Error("[block.CreateBlockAndUpdateMempool] %s", dbTx.Error)
				return dbTx.Error
			}
			if dbTx.RowsAffected == 0 {
				logx.Error("[block.CreateBlockAndUpdateMempool] Create Invalid Block")
				return ErrInvalidBlock
			}

			// Update tx.BlockId and block.Txs
			for _, tx := range txs {
				tx.BlockId = int64(block.ID)
			}
			block.Txs = txs

			dbTx = tx.Table(m.table).Save(block)
			if dbTx.Error != nil {
				logx.Error("[block.CreateBlockAndUpdateMempool] %s", dbTx.Error)
				return dbTx.Error
			}
			if dbTx.RowsAffected == 0 {
				logx.Error("[block.CreateBlockAndUpdateMempool] Update Invalid Block")
				return ErrInvalidBlock
			}

			// Update Mempool
			for _, mempoolTx := range mempoolTxs {
				mempoolTx.Status = mempool.HandledTxStatus
				...
				if dbTx.RowsAffected == 0 {
					logx.Error("[block.CreateBlockAndUpdateMempool] Update Invalid Mempool Tx")
			...
			// Update total Amount
			//for chainId, v1 := range totalAmountMap {
			//	for assetId, v2 := range v1 {
			//		dbTx := tx.Table(l1amount.TableName).Create(&l1amount.L1Amount{
			//			ChainId:     uint8(chainId),
			//			AssetId:     uint32(assetId),
			//			TotalAmount: int64(v2),
			...
			//		}
			//		if dbTx.RowsAffected == 0 {
			//			logx.Error("[block.CreateBlockAndUpdateMempool] Create Invalid L1 Amount")
...
func (m *defaultBlockModel) GetBlockStatusCacheByBlockHeight(blockHeight int64) (blockStatusInfo *BlockStatusInfo, err error) {
	...
	if err != nil {
		...
		if err != nil {
			logx.Errorf("[tx.GetBlockStatusCacheByBlockHeight] json.Unmarshal error: %s, value : %s", err.Error(), blockStatusInfoFromCache)
			return nil, err
		}
	}
