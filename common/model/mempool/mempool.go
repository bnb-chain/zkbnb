...
func (m *defaultMempoolModel) GetL1MempoolTx(chainId int64, assetId int64) (mempoolTxs []*MempoolTx, err error) {
	dbTx := m.DB.Table(m.table).Where("status = ? and chain_id = ? and asset_a_id = ? and (tx_type = ? or tx_type = ?)",
		PendingTxStatus, chainId, assetId, commonTx.TxTypeDeposit, commonTx.TxTypeWithdraw).Find(&mempoolTxs)
	if dbTx.Error != nil {
