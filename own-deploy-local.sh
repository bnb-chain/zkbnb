...
cp -r /path/to/zkbnb-crypto ${DEPLOY_PATH}
...
if [ $flag = "new" ]; then
  ...
  cd ${DEPLOY_PATH}
  cd zkbnb-crypto && go test ./circuit/solidity -timeout 99999s -run TestExportSolSmall
  cd ${DEPLOY_PATH}
  mkdir -p $KEY_PATH
...
EOF
yarn install
sed -i "" "s/gas: 15000000/gas: 10000000/g" ./hardhat.config.js
...
KeyPath:
  ProvingKeyPath: [${KEY_PATH}/zkbnb1.pk]
  VerifyingKeyPath: [${KEY_PATH}/zkbnb1.vk]
